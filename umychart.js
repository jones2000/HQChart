...
function JSChart(divElement)
{
    ...
    this.CreateKLineChartContainer=function(option)
    {
        ...
        chart.RightMenu=new KLineRightMenu(this.DivElement);
    ...
    this.ChangeRight=function(right)
    {
        if (this.JSChartContainer && typeof(this.JSChartContainer.ChangeRight)=='function')
            this.JSChartContainer.ChangeRight(period);
    }
}
...
function JSChartContainer(uielement)
{
    ...
    this.FrameSplitData=new Map([["double",new SplitData() ],["price",new PriceSplitData()]]);
    ...
    uielement.ontouchstart=function(e)
    {
        ...
        if (IsPhoneDragging(e))
        {
            ...
            drag.Click.X=e.touches[0].clientX;
            drag.Click.Y=e.touches[0].clientY;
            drag.LastMove.X=e.touches[0].clientX;
            drag.LastMove.Y=e.touches[0].clientY;
        ...
        else if (IsPhonePinching(e))
        {
            ...
            phonePinch.Start={"X":e.touches[0].pageX,"Y":e.touches[0].pageY,"X2":e.touches[1].pageX,"Y2":e.touches[1].pageY};
            phonePinch.Last={"X":e.touches[0].pageX,"Y":e.touches[0].pageY,"X2":e.touches[1].pageX,"Y2":e.touches[1].pageY};
        ...
        uielement.ontouchmove=function(e)
        {
            ...
            if (IsPhoneDragging(e))
            {
                ...
                if (drag==null)
                {
                    var x = e.touches[0].clientX-this.getBoundingClientRect().left;
                    var y = e.touches[0].clientY-this.getBoundingClientRect().top;
                    this.JSChartContainer.OnMouseMove(x,y,e);
                }
                else
                {
                    var moveSetp=Math.abs(drag.LastMove.X-e.touches[0].clientX);
                    ...
                    if (this.JSChartContainer.DragMode==1)  //数据左右拖拽
                    {
                        ...
                        var isLeft=true;
                        if (drag.LastMove.X<e.touches[0].clientX) isLeft=false;//右移数据
            ...
            }else if (IsPhonePinching(e))
            {
                ...
                var yHeight=Math.abs(e.touches[0].pageY-e.touches[1].pageY);
 ...
 //   this.SelectRectAlpha=0.06;                  //透明度
    ...
    this.KLine={
            ...
            Info:  //信息地雷
            {
                ...
                Pforecast:  //业绩预告
                {
                    ApiUrl:'http://opensource.zealink.com/API/StockHistoryDay',
                ...
                {
                    ApiUrl:'http://opensource.zealink.com/API/StockHistoryDay',
                    Icon:'http://beigo.oss-cn-beijing.aliyuncs.com/cache/test/blocktrading.png',
                },
                TradeDetail:    //龙虎榜
                {
                    ApiUrl:'http://opensource.zealink.com/API/StockHistoryDay',
                    Icon:'http://beigo.oss-cn-beijing.aliyuncs.com/cache/test/tradedetail.png',
                }
    ...
    //历史数据api
    this.Index.StockHistoryDayApiUrl="http://opensource.zealink.com/API/StockHistoryDay";
    //市场多空
    this.Index.MarketLongShortApiUrl="http://opensource.zealink.com/API/FactorTiming";
    //市场关注度
    this.Index.MarketAttentionApiUrl="http://web4.umydata.com/API/MarketAttention";
    //行业,指数热度
    this.Index.MarketHeatApiUrl="http://web4.umydata.com/API/MarketHeat"
...
function KLineChartContainer(uielement)
{
    ...
    this.RealtimeApiUrl="http://opensource.zealink.com/API/Stock";                    //实时行情api地址
...
function MinuteChartContainer(uielement)
{
    ...
    this.MinuteApiUrl="http://opensource.zealink.com/API/Stock";
...
function HistoryMinuteChartContainer(uielement)
{
    ...
    this.HistoryMinuteApiUrl="http://opensource.cache.umydata.com/cache/minuteday/day/";
...
function MarketHeatIndex()
{
    ...
    this.Name='Market-Heat';
    this.ApiUrl=g_JSChartResource.Index.MarketHeatApiUrl;
    this.Index=new Array(
        new IndexInfo("指数热度(%)",null)
    );

    //调整框架
    this.SetFrame=function(hqChart,windowIndex,hisData)
    {
        hqChart.Frame.SubFrame[windowIndex].Frame.YSpecificMaxMin={Max:100,Min:0,Count:3};
...
function ChangeIndexDialog(divElement)
{
    ...
    this.DivElement=divElement;   //父节点
    this.IndexTreeApiUrl="http://opensource.cache.umydata.com/cache/hqh5/index/commonindextree.json";      //数据下载地址
        ...
        for(var i in infoList)
        {
            ...
            if(infoType==KLINE_INFO_TYPE.PFORECAST)
            {
                ...
                if (item.ExtendData.FWeek)
                {
                    ...
                    if (weekData.length>0) weekData='<br>&nbsp;&nbsp;'+weekData;
                }
